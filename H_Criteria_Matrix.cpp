...
// [[Rcpp::export]]
NumericMatrix max_heapify_c(NumericMatrix A, int i, int heap_size) {
  A = clone(A);
  // Left child index
  int l = 2 * i + 1; 
  // Right child index
  ...
  if (largest != i) {
    ...
    A = max_heapify_c(A, largest, heap_size);
  }
